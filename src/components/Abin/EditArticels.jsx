...
const EditArticle = ({ article, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/articles/${article._id}`, {
        method: "PUT",
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to update article document");
      }

      alert("Article structural fields updated successfully!");
      if (onSave) onSave(data);
    } catch (error) {
      console.error("Update Error Pipeline Error:", error);
